...
const Transactions = ({ projectId: propProjectId }) => {
  ...
  const handleSaveTransaction = async (e) => {
    ...
    if (!formData.title.trim() || isNaN(parsedAmount) || parsedAmount <= 0) {
      setErrorMessage('Please provide a valid entry title and a positive monetary amount.');
      return;
    }
  ...
  const categoriesList = useMemo(() => {
    const cats = new Set([...DEFAULT_CATEGORIES, ...transactions.map((t) => t.category).filter(Boolean)]);
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex flex-col lg:flex-row items-center justify-between gap-4 shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
          ...
          <select
            ...
            className="bg-slate-800 border border-slate-700 text-slate-300 px-3 py-2 rounded-xl text-xs font-medium focus:outline-none focus:border-blue-500 cursor-pointer"
          >
