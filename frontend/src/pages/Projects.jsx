...
// Fallback initial data in case the backend returns empty or is offline
...
const Projects = () => {
  ...
  // Fetch projects list from Django REST backend
  const fetchProjects = async () => {
    setLoading(true);
    setErrorMessage(null);
    try {
      const response = await API.get('projects/');
      // Handle array or paginated response format from Django REST Framework
      ...
      console.warn('Backend endpoint offline or unreachable. Using fallback projects state.', err);
      setErrorMessage('Could not load online records. Showing local preview data.');
      setProjects(defaultProjects);
    } finally {
      setLoading(false);
    }
  ...
  // Handle Project Deletion via API
  ...
  // Helper badge color picker based on project status
  ...
  return (
    <div className="space-y-8">
      {/* Backend connection warning notice if any */}
      {errorMessage && (
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Manage, evaluate, and track your financial feasibility studies.
          </p>
        </div>
      ...
      {filteredProjects.length === 0 ? (
        <div className="flex flex-col items-center justify-center bg-slate-900 border border-slate-800 rounded-2xl p-12 text-center">
          ...
          <p className="text-sm text-slate-400 mt-1 max-w-sm">
            Try adjusting your search criteria or create a new feasibility study to get started.
          </p>
        </div>
      ) : (
          ...
          {filteredProjects.map((project) => (
            ...
            >
              <div>
                ...
                {/* Metrics Highlights */}
                <div className="grid grid-cols-2 gap-3 my-5 p-3 bg-slate-800/50 rounded-xl border border-slate-800">
                  <div>
                    <span className="text-[10px] uppercase font-semibold text-slate-500 block">CapEx / Inv.</span>
                    <span className="text-sm font-bold text-slate-200 flex items-center mt-0.5">
                      <DollarSign size={12} className="text-slate-400" />
                      {Number(project.initialInvestment || 0).toLocaleString()}
                    </span>
                  </div>
                  <div>
                    <span className="text-[10px] uppercase font-semibold text-slate-500 block">Est. IRR</span>
